...
async function main() {
  ...
  console.log("Swapping 1.82 USDC to MATIC");
  result = await lib.Swap(lib.Tokens.USDC, lib.Tokens.MATIC, 1.82, false);
  console.log(`Result: ${result}\n`);

  console.log("After (we should have 1.82 USDC less and more MATIC in the wallet) :\n");
  balance = await lib.GetAmount(lib.Tokens.USDC);
  if (balance !== false) {
    console.log(`Balance: ${balance.value} USDC`);
  }
